...
import {
  Paper, Box, Typography, Accordion, AccordionSummary, AccordionDetails,
  Button, Divider, Dialog, DialogTitle, DialogContent, DialogContentText,
  DialogActions, IconButton, CircularProgress, Chip, Table, TableBody,
  TableCell, TableContainer, TableHead, TableRow, TextField, MenuItem,
  FormControlLabel, Switch
} from "@mui/material";
import { 
  ExpandMore, Visibility, Add, Edit, Delete, Check, Close
...
const StudentProgress = () => {
  ...
  const [outcomeDialog, setOutcomeDialog] = useState({
    open: false,
    isEdit: false,
    data: { topic: "", credits: 1, compulsory: true, requirements: [] },
    subjectId: null,
    outcomeId: null
  });
  ...
  const handleEditSubject = (subject) => {
    setSubjectDialog({
      open: true,
      isEdit: true,
      data: { name: subject.name, credits: subject.credits },
  ...
  return (
    <Box sx={{ padding: 3 }}>
              ...
              >
                ...
                >
                  ...
                  <Box>
                    ...
                    <IconButton 
                      color="error" 
                      onClick={() => handleDeleteSubject(subject.subjectId, subject.name)}
                    >
                      <Delete />
                    </IconButton>
                  </Box>
                      ...
                      <Accordion key={outcome.outcomeId}>
                        <AccordionSummary expandIcon={<ExpandMore />}>
                          <Box sx={{ display: "flex", alignItems: "center", width: "100%" }}>
                            <Typography sx={{ flexGrow: 1 }}>
                              {outcome.topic} ({outcome.credits} credits)
                            </Typography>
                            <Chip 
                        ...
                        <AccordionDetails>
                          ...
                          <Box sx={{ display: "flex", mb: 2, gap: 1 }}>
                            ...
                            <IconButton 
                              color="primary" 
                              onClick={() => handleEditOutcome(outcome, subject.subjectId)}
                            >
                              <Edit />
                            </IconButton>
                                    ...
                                    <TableRow key={project._id}>
                                      ...
                                      <TableCell>
                                        {project.status === "Approved" ? 
                                          (project.approvedCredit || project.requestedCredit) : "-"}
                                      </TableCell>
                                      <TableCell>
                                        {project.assessedBy || "-"}
                                      </TableCell>
                                      <TableCell>
                                        <IconButton
                                          color="primary"
                                          size="small"
                                          onClick={() => handleProjectAssessment(project, outcome, subject)}
                                        >
                                          <Edit />
                                        </IconButton>
      ...
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            type="number"
            fullWidth
            value={subjectDialog.data.credits}
            onChange={(e) => setSubjectDialog({
              ...subjectDialog,
              data: { ...subjectDialog.data, credits: Number(e.target.value) }
            })}
        ...
        <DialogActions>
          ...
          <Button 
            ...
            variant="contained" 
            color="primary"
            disabled={!subjectDialog.data.name || !subjectDialog.data.credits}
          >
            Save
          </Button>
        </DialogActions>
      ...
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            type="number"
            fullWidth
            value={outcomeDialog.data.credits}
            onChange={(e) => setOutcomeDialog({
              ...outcomeDialog,
              data: { ...outcomeDialog.data, credits: Number(e.target.value) }
            })}
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => setOutcomeDialog({
                  ...outcomeDialog,
                  data: { ...outcomeDialog.data, compulsory: e.target.checked }
                })}
                color="primary"
              />
            }
        ...
        <DialogActions>
          ...
          <Button 
            ...
            variant="contained" 
            color="primary"
            disabled={!outcomeDialog.data.topic || !outcomeDialog.data.credits}
          >
            Save
          </Button>
        </DialogActions>
      ...
      <Dialog 
        ...
        onClose={() => setRequirementDialog({ ...requirementDialog, open: false })}
        fullWidth
        maxWidth="md"
      >
        ...
        <DialogContent>
          <TextField
            ...
            onChange={(e) => setRequirementDialog({
              ...requirementDialog,
        ...
        <DialogActions>
          <Button 
            onClick={() => setRequirementDialog({ ...requirementDialog, open: false })}
          >
            Cancel
          </Button>
          <Button 
      ...
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            onChange={(e) => setProjectDialog({
              ...projectDialog,
              data: { ...projectDialog.data, approvedCredit: Number(e.target.value) }
            })}
            InputProps={{ inputProps: { min: 0.1, step: 0.1 } }}
