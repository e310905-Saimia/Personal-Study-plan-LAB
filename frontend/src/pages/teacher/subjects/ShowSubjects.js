...
const [updatingRequirements] = useState(false);
  ...
  const handleUpdateRequirement = () => {
    ...
    dispatch(
        updateOutcome(selectedOutcome.subjectId, selectedOutcome._id, { requirements: updatedRequirements })
    ).then(() => {
  ...
  return (
    <>
      ...
      <Dialog open={openRequirements} onClose={() => setOpenRequirements(false)} fullWidth maxWidth="md">
        ...
        <DialogContent>
          <TextField fullWidth multiline rows={6} variant="outlined" value={requirementText} onChange={(e) => setRequirementText(e.target.value)} />
        </DialogContent>
        <DialogActions>
